...
def draw_board(board, offset_x=DEFAULT_OFFSET_X, offset_y=DEFAULT_OFFSET_Y, square_size=SQUARE_SIZE):
    ...
    for i in range(8):
        t = FONT.render(files[i], True, (255, 255, 255))
        screen.blit(t, (offset_x + i * square_size + 20, offset_y + 8 * square_size))
    for i in range(8):
        t = FONT.render(ranks[7 - i], True, (255, 255, 255))
        screen.blit(t, (offset_x - 20, offset_y + i * square_size + 10))
...
def play_against_ai():
    ...
    while running:
        ...
        screen.blit(move_text, (50, HEIGHT - 605))
        pygame.display.flip()
